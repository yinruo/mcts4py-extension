...
class GenericSolarisSolver(MCTSSolver[TAction, NewNode[TRandom, TAction], TRandom], Generic[TState, TAction, TRandom]):
    ...
    def run_game(self, episodes: int):
        ...
        for e in range(episodes):
            ...
            root_node = self.root()
            game = gym.make('ALE/Riverraid-v5')
            game.reset()
            root_node.state.current_state = game.unwrapped.clone_state(include_rng=True)
            root_node.state.is_terminal = False
