...
class SolverOption(MCTSSolver[TAction, NewNode[TRandom, TAction], TRandom], Generic[TState, TAction, TRandom]):
    ...
    def run_option(self):
        ...
        self.run_iteration(root_node, 10000000)
    ...
    def simulate(self, node: ActionNode[TState, TAction], depth=0) -> float:
        ...
        print("马上执行价格", immediate_payoff)
        print("time step", node.state.time_step)
        print("期望价格", future_expected_reward)
    ...
    def print_tree(self, node: ActionNode[TState, TAction], level=0, is_last=True, prefix=""):
        ...
        print(f"{indent}资产价格: {node.state.asset_price:.4f}, 期权价格: {node.reward:.4f}, terminal:{node.state.is_terminal}, time: {node.state.time_step} ")
